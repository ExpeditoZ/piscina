...
export function PoolDetailClient({ pool }: PoolDetailClientProps) {
  ...
  // CTA button opens checkout or scrolls to calendar
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-2xl lg:max-w-4xl mx-auto">
        {/* Image Carousel — edge-to-edge on mobile */}
        <div className="sm:px-4 sm:pt-4">
          <ImageCarousel images={pool.photos} title={pool.title} />
        </div>

        {/* Content with safe bottom padding for sticky CTA */}
        <div className="px-4 pt-5 pb-28 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-white rounded-xl p-3.5 border border-slate-100 text-center">
            ...
            <div className="bg-orange-50/70 rounded-xl p-3.5 border border-orange-100 text-center">
              <p className="text-[10px] font-semibold uppercase tracking-widest text-orange-500 mb-0.5">
                Fim de semana
              </p>
          ...
          {hasShifts && (
            <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
              <div className="px-4 pt-3.5 pb-2 flex items-center gap-2">
                ...
                <h2 className="font-bold text-[14px] text-slate-800">
                  Turnos Disponíveis
                </h2>
              </div>
      ...
      {!checkoutOpen && (
        <div
          className="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-slate-200 shadow-[0_-4px_20px_rgba(0,0,0,0.06)]"
          style={{ paddingBottom: "env(safe-area-inset-bottom, 0px)" }}
        >
          <div className="max-w-2xl lg:max-w-4xl mx-auto px-4 py-2.5">
            <div className="flex items-center gap-3">
              ...
              {/* CTA Button — prominent, tall enough for thumbs */}
              <button
                onClick={handleOpenCheckout}
                className={`flex-shrink-0 flex items-center justify-center gap-2 h-12 px-6 rounded-xl font-bold text-[14px] shadow-md transition-all duration-200 active:scale-[0.96] ${
                  selectedDate
                    ? "bg-emerald-500 hover:bg-emerald-600 text-white shadow-emerald-200/60"
                    : "bg-sky-500 hover:bg-sky-600 text-white shadow-sky-200/60"
                }`}
              >
                {selectedDate ? (
                  <>
